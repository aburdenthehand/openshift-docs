...
When you apply a node network policy, a `NodeNetworkConfigurationEnactment` is created for every node in the cluster. The node network enactment is a read-only object that represents the status of execution of the policy on that node.
